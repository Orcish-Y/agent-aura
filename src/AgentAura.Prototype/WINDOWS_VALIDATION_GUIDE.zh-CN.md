...
- 示例 Agent Item 可从一行展开为四行，提供状态图形、低饱和颜色、编辑/删除、长文本省略号裁切和详情淡入。
...
5. 检查较长的标题或详情文本。确认它始终在可用宽度内以省略号裁切，没有内部滚动，也没有工具提示。

通过标准：悬停从一行变为四行，状态有形状/文字与颜色双重表达，长文本始终省略号裁切且没有工具提示。
