...
这份手册用于完成 `Validate the Windows observation shell` 票据的实机验证。完成后，原型应确认所需核心能力是否能由 WPF 实现，并记录 WPF 的资源与稳定性基线。
...
所有“核心能力”门槛必须通过。资源或稳定性门槛失败时，应记录证据并在 WPF 内调查和优化。只有必需的核心能力确认无法在 .NET/WPF 中实现时，才启动同等范围的 Tauri 对比。
...
3. 检查状态不只依赖颜色：Attention 显示 `!`，Running 显示 `▶`，Completed 显示 `✓`；颜色应为低饱和色。
...
在验证表的 `Decision record` 中写出结论：

- **继续使用 WPF**：全部核心能力通过。写明实际资源与稳定性测量、未达到的轻量目标、优化方向和已知限制。
- **启动 WPF 调查**：资源、稳定性或可修复的能力关卡失败时，列出失败门槛、复现步骤、环境、证据和影响。
- **考虑 Tauri 对比**：仅当必需的核心能力确认无法在 .NET/WPF 中实现时使用。对比必须覆盖同一套托盘、窗口、无障碍和框架依赖分发能力，不能只比较包体大小。

没有实机数据时，不能仅因项目能够编译或发布就声称核心能力已经通过，也不能编造资源结论。
